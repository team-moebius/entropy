...
public class TradeWindowVolumeResolver {
	...
	public BigDecimal getRandomMarketVolume(BigDecimal minVolume, BigDecimal maxVolume,
		int decimalPosition) {
		return randomUtils
			.getRandomDecimal(minVolume.floatValue(), maxVolume.floatValue(), decimalPosition);
	}
	...
	public List<BigDecimal> getDividedVolume(DividedDummyOrderDto orderDto,
		...
		BigDecimal inflationVolume = Optional.ofNullable(orderDto)
			...
			.map(inflationConfig -> getRandomVolume(inflationConfig, orderPosition,
				orderDto.getMarket().getVolumeDecimalPosition()))
			.orElse(BigDecimal.ZERO);
		...
		for (int i = range - 1; i > 0; --i) {
			BigDecimal dividedVolume = randomUtils
				.getRandomDecimal(4f * i, inflationVolume.floatValue(),
		...
		if (inflationVolume.compareTo(BigDecimal.valueOf(4)) < 0) {
			dividedVolumes.add(BigDecimal.valueOf(4));
		} else {
